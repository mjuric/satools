...
* Selected points
 ...
 203  FORMAT(4X,A,' observations:',3I6,' (DT=',2F10.2,' d)')
      ...
      IF(iodvrb.GE.2) THEN
          write(99,204) ir,tdt(ir(2))-tdt(ir(1))
...
* LOOP 3 (on different methods)
 ...
 2        CONTINUE
          ...
          IF(iodvrb.GE.2) THEN
              lmsg=lench(msg)
              IF(lmsg.GT.0) THEN
                  write(99,220) 'Gauss',name(1:ln),ngr,nsol,nkep,
     +                         msg(1:lmsg)
                  IF(unirep.GT.0) WRITE(unirep,220) 'Gauss',name(1:ln),
     +                            ngr,nsol,nkep,msg(1:lmsg)
              ELSE
                  write(99,221) 'Gauss',name(1:ln),ngr,nsol,nkep
 ...
 3        CONTINUE
      ...
      ELSEIF(iodmet(itry).EQ.2) THEN
          nc(6)=nc(6)+1
          CALL vaisala(tdt3,alpha3,delta3,obsc3,ifo,elemv,t0v,fail)
          nsol=1
          IF(fail) nsol=0
          nc(7)=nc(7)+nsol
          IF(iodvrb.GE.2) THEN
              write(99,221) 'Vaisala',name(1:ln),nsol,nsol,nsol
     ...
     +                iobs,obscod,sel,ir(1),ir(2),resa,resd,rms1)
          IF(iodvrb.GE.2) THEN
              IF(ieltyv(is).EQ.'KEP') THEN
                  write(99,222) 'Vaisala',name(1:ln),is,'a',elemv(1,is),
     +                         elemv(2,is),rms1*secrad
                  IF(unirep.GT.0) WRITE(unirep,222) 'Vaisala',
     +                            name(1:ln),is,'a',elemv(1,is),
     +                            elemv(2,is),rms1*secrad
              ELSE
                  write(99,222) 'Vaisala',name(1:ln),is,'q',elemv(1,is),
     +                          elemv(2,is),rms1*secrad
                  IF(unirep.GT.0) WRITE(unirep,222) 'Vaisala',
...
* END OF LOOP 1
 ...
 20   CONTINUE
      IF(existb) THEN
          ...
          write(99,230) name(1:ln),nc,selb,irb,dt1,dt2,dtc
          IF(unirep.GT.0) WRITE(unirep,230)
     +                        name(1:ln),nc,selb,ir,dt1,dt2,dtc
      ELSE
          write(99,235) name(1:ln),nc
 ...
 235  FORMAT(4X,'IOD(',A,') STAT:',7I6)
      ...
      IF(existb) THEN
          lm=lench(methb)
          IF(in.LE.0) THEN
              write(99,232) name(1:ln),methb(1:lm),rmsb*secrad
              IF(unirep.GT.0) WRITE(unirep,232) name(1:ln),methb(1:lm),
     +                                          rmsb*secrad
              comele=methb(1:lm)
          ELSE
              write(99,233) name(1:ln),methb(1:lm),rmsb*secrad
