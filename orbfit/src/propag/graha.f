...
c dimension check
      if(n.gt.nx)then
         write(99,*)'n =',n,' larger than nx=',nx,' in graha'
         stop
      endif 
...
c V1 is the versor of A1
      ...
      if(ize)then
         write(99,*)' first vector of a is too small'
c        stop
      endif 
...
c by removing the components along the previous ones
      ...
      if(ize)then
         write(99,*)' a has practically rank one'
c        stop
      endif
...
c the new versor is a good one
 ...
 2    continue
      if(jok.lt.n-2)then
         write(99,*)' something went wrong, jok=',jok
      endif
      return
      end
