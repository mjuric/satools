...
c initialisation at the beginning of each close approach
      IF(first)THEN
         ...
         r1=vsize(x)
         rdot1=prscal(x,v)/r1    
C        write(99,*) ' close approach to planet ',ordnam(iplam)
C        write(99,*) ' beginning t,r,rdot=',tt1,r1,rdot1
...
c compute r, rdot
      ...
      rdot2=prscal(x,v)/r2
      tt2=t
c     write(99,*)tt2,r2,rdot2
      IF(tt2.gt.tt1)THEN
         di=1.d0
      ELSEIF(tt2.lt.tt1)THEN
         di=-1.d0
      ELSE
         write(99,*)' falsi: zero step'
         write(99,*)tt1,tt2,x,v,xpla
      ENDIF
c     write(99,*)rdot1,rdot2
      ...
      ELSEIF(rdot1*di.lt.0.d0.and.rdot2*di.gt.0.d0)THEN
         t1=tt1
         t2=tt2
*        write(99,*) ' r. f. begins, t,r,rdot=',t1,r1,rdot1,t2,r2,rdot2
...
c           IF(abs(rdot0).lt.eprdot)THEN
           ...
           tt=dt+t2
           hh=tt-t
C         write(99,111)t1,r1,rdot1,t2,r2,rdot2,dt,tt
...
c        write(99,*)z1,zdot1,z2,zdot2
         ...
         ELSEIF(z1*di.lt.0.d0.and.z2*di.gt.0.d0)THEN
            t1=tt1
            t2=tt2
c      write(99,*) ' r. f. begins, t,z,zdot=',t1,z1,zdot1,t2,z2,zdot2
...
c planetocentric position
              ...
              tt=dt+t2
              hh=tt-t
c           write(99,*)t1,z1,zdot1
c           write(99,*)t2,z2,zdot2,dt,tt,di,it
            ENDDO
c failed convergence
            write(99,*)' falsi: failed mtp convergence'
            write(99,*)t1,z1,zdot1
            write(99,*)t2,z2,zdot2,dt,tt,di,itmax
         ENDIF
      ENDIF
 3    CONTINUE
...
c store close approach point
      jt=jt+1
      IF(jt.gt.njtx)THEN
          write(99,*)' strmtp: jt>njtx ',jt,njtx
          STOP
      ENDIF
      ttar(jt)=tcur
      DO i=1,3
        ...
        vtp(i,jt)=v(i)
      ENDDO
c     write(99,*) ' target plane at time, distance, zdot'
c     write(99,*) tcur, z, zdot
...
c store close approach point
      jc=jc+1
      IF(jc.gt.njcx)THEN
          write(99,*)' strclo: jc>njcx ',jc,njcx
          STOP
      ENDIF
      tcla(jc)=tcur
      rmin(jc)=r
      DO i=1,3
        ...
        vcla(i,jc)=v(i)
      ENDDO
c     write(99,*) ' closest approach at time, distance, rdot'
...
!      write(99,97)planam(1:lpla),date,tcur,r
...
c  control on end of gauss-seidel iterations
      ...
      IF(it.gt.1.and.ep(it).gt.ep(it-1)*1.1d0)THEN
         write(99,*)' rkg: stop at iteration ',it, ' before too late'
         write(99,*)t1,(ep(jj),jj=it-1,it)
         GOTO 77
      ENDIF
...
c  too many gauss-seidel iterations
            write(99,*)' rkg: non convergent after ',it,' iterations'
            write(99,*)t1,ep(it)
         ENDIF
         it=it+1
         GOTO 1
