...
c compute intermediate point (first guess)        
        ...
        ENDDO 
c       write(99,*)'eq12, iter.0 ',(eq12(j),j=1,3)
...
c recompute intermediate point
           ...
           ENDDO 
c          write(99,*)'weak sigma, iter, eq12',sdir12,it,(eq12(j),j=1,3)
c before a new iteration, tests that orbit is still elliptic
           e12=sqrt(eq12(2)**2+eq12(3)**2)
           IF(e12.gt.0.99d0.or.eq12(1).le.0.d0)THEN
              fail=.true.
              call vcopy(6,eq12,eq1)
c               write(99,*)'intstep: fail at iteration ',it,fail
              RETURN
           ELSEIF(bizarre(eq12))THEN
              fail=.true.
              call vcopy(6,eq12,eq1)
...
c          write(99,*)' intstep: it, ang, dsi, sdir12,sdirst ',
c    +          it,ang,dsi,sdir12,sdirst
           IF(ang.lt.eps.and.dsi.lt.eps)THEN
c             write(99,*)' intstep: it, ang, dsir ',it,ang,dsi
...
c       write(99,*)' intstep: failed convergence, it, ang, dsig'
c       write(99,*)it-1,ang,(sdir12-sdirst)/sdirst
        fail=.true.
        call vcopy(6,eq12,eq1)
        RETURN
...
c but we go on
      ELSE
         write(99,*)' intstep: not invented yet, imint=',imint
         STOP
      ENDIF
c     write(99,*)'intstep: at exit ',fail, (eq1(j),j=1,3)
      RETURN
      END
