 ...
 101     FORMAT(5x,a1)
     ...
     +        col.eq.'I'.or.col.eq.' '))THEN
            IF(ierrou.gt.0)THEN
               ...
               numerr=numerr+1
            ELSE
               write(99,*) 'Unknown Color:',col,' Obs #',j
            ENDIF
         ENDIF
         IF(avail(j))THEN
...
c jump out if no good observations
         IF(icount.eq.0)THEN
            write(99,*)' magnitude cannot be estimated w/o obs'
            rmsh=-1.d0
            RETURN
         ENDIF             
c Compute H, etc.
...
C Divide by icount or wsum here?
         ...
         IF(verbose) write(99,*)'H,RMS',hmean,rmsh
...
C              test for recovery
     ...
     +                 (wsum+1.d0/tmprms(j))
                  ...
                  IF(chi2.lt.x2mrec)THEN
                     ...
                     rmsmag(j)=tmprms(j)
                  ENDIF
                  IF(verbose) write(99,*)'REC:Hnew,chi2,rms,res',
...
C              test for rejection
     ...
     +                 (wsum-1.d0/rmsmag(j))
                  ...
                  IF(chi2.gt.x2mrej)THEN
                     ...
                     rmsmag(j)=99.99d0
                  ENDIF
                  IF(verbose) write(99,*)'REJ:Hnew,chi2,rms,res',
     +                 hnew,chi2,rmsmag(j),resmag(j)
               ENDIF
C              count rejection
      ...
      ENDDO
      write(99,*)'WARNING: Did not finish rejecting photometry...'
************************************************************************
 10   IF(.not.batch)THEN
         write(99,190)h0,hmean,icount,rmsh,numrej,nrej
...
c parsing error
 3    write(99,*)'magest: error in parsing magn. no ',j,' ',smag(j)
      STOP
      END
