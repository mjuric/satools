...
*     Next set minobs
      ...
      IF(nobs.le.minobs)THEN
         write(99,*)'No autorejection with only ',nobs,' observations.'
         nmod=0
         RETURN
      ENDIF
...
* Chi-square value
         ...
         ELSE
            write(99,*)'reject: this should not happen ',
     +           w(ipa),' ',w(ipd)
            STOP
         ENDIF
         IF (x2(iob).lt.0.d0)THEN
            WRITE(ierrou,*) 'WARNING: reject.f, x2=',x2(iob)
            write(99,*) 'WARNING: reject.f, x2=',x2(iob)
            numerr=numerr+1
            x2(iob)=0.d0
         ENDIF
...
*     For <=18 obs. or for inf. loop -> reject one at a time
      IF(nsel .le. minobs*6) THEN
         IF(log) write(99,*) 'Rejecting no more than one.'
      ELSEIF(nit.ge.4) THEN
         IF(log) write(99,*) 'Trying to get unstuck.'
      ELSE
         x2max=x2max*x2frac
      ENDIF
      iun=abs(unilog)
      WRITE(iun,302) x2max
      IF(log) write(99,302) x2max
...
c check to see if this is the only remaining obs in an opposition
 ...
 301  FORMAT(I5,1P,E13.5,0P,2I3)
      ...
      IF(log) write(99,303) nobs,nsel,nrej,nrec
