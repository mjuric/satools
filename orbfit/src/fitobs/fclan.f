...
c t1 has been passed as dummy variable, nothing to do
         ELSE
c select final time
 56         WRITE(*,*)' search for close approaches until time (MJD)?'
            READ(*,*) t1
            IF(icont.eq.1.and.(t1-tc)/(t1old-tc).le.1.d0)THEN
               WRITE(*,*)' change in direction not allowed '
               WRITE(*,*)' epoch, old target time, new target time' 
               WRITE(*,*) tc,t1old,t1,' must be in sequence'
               GOTO 56
            ENDIF
         ENDIF 
...
c for previously selected target plane: what to do? nothing
         WRITE(*,*)' using previously selected target plane'
         WRITE(iun20,*)' using previously selected target plane'
         WRITE(*,*)' close app. at t=',tcl0
         WRITE(*,*)' distance=',dmin0,' velocity=',vmin0
      ELSE
         WRITE(*,*)' fclan: option iclan=',iclan,' not available'
         RETURN
      ENDIF
c =============================================================
...
c     ELSE
c        WRITE(*,*)' fclan: option iclan=',iclan,' not available'
c        RETURN
c     ENDIF     
c =====================================================================
c options
...
c           WRITE(*,*)' go on with search for close app.? 1=yes 0=no'
c           READ(*,*)icont
...
c iteration step acceptable?
         IF(error)THEN
            write(*,*)' Newton failed at iteration ',itnew
