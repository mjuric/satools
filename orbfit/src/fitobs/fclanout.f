...
c check for existence of data
     ...
     +             (tta.gt.tcl2.and.tta.lt.tcl1))THEN
              ELSE
                 write(99,*)' point ',n, ' time at target plane ',
     +              tta,' was ', tcl0
              ENDIF
              nt=nt+1
...
c file output: target plane
 ...
 107          FORMAT(7e20.12)
c temporary output
              write(99,177)ttar(jt),xta(nt),yta(nt),nt,n
 177          FORMAT(f11.4,1x,f7.4,1x,f7.4,1x,i4,1x,i4)
              IF(njt.gt.1)THEN
                 write(99,*)' warning: multiple intersection ',njt,n
              ENDIF
c reset counter for next orbit
              njt=0
           ELSEIF(njt.le.0)THEN
              write(99,*)' no target plane ',n
              GOTO 7
           ENDIF
c close approach manifold
...
c true close approach manifold plotted on the MTP
                 ...
                 ycl(nc)=xx(3)*rr
c temporary output
                 write(99,177)tcla(jc),xcl(nc),ycl(nc),nc
                 write(99,*)
                 IF(njc.gt.1)THEN
                   write(99,*)' warning: multiple approach ',njc
                 ENDIF
c reset counter
                 njc=0
              ENDIF
           ELSEIF(njc.le.0)THEN
              write(99,*)' no minimum distance ',n
              GOTO 7
           ENDIF
        ELSE
           write(99,*)' fclan:this we have not invented yet ', inl
           RETURN           
        ENDIF
 7    continue
      CALL filclo(iun7,' ')
...
c mark Earth
     ...
     +     xlabel,ylabel,title,idev,1)
      ELSE
         write(99,*)' no target plane point available', npot
      ENDIF
      IF(inl.eq.1)GOTO 2
...
c If we are making .ps files then the MTP will be overwritten!
      if(idev.eq.5.or.idev.eq.6)then
         write(99,*)'The PostScript file ''giffv.ps'' is '//
