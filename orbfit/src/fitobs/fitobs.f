...
c multiple solutions have been propagated (without time update?)
      LOGICAL propti
      DOUBLE PRECISION trmult,tcmult
c ======== output elements =====================
...
c =====================================================================
         IF(iprop.eq.1.or.iprop.eq.4)THEN
            CALL tee(iun20,' ARC 1=')
            CALL fstpro(icov,ini0,cov0,iun20,iun8,ok,
     ...
     +                  c0,cov0,h0,gma0,0.d0)
         ...
         ELSEIF(iprop.eq.2.or.iprop.eq.5)THEN
            CALL tee(iun20,' ARC 2=')
            CALL fstpro(icov,inip,covp,iun20,iun8,ok,
     ...
     +                  cp,covp,hp,gmap,0.d0)
         ...
         ELSEIF(iprop.eq.3)THEN
            CALL tee(iun20,' BOTH ARCS=')
            CALL fstpro(icov,initwo,covtwo,iun20,iun8,ok,
...
c MENU 6: PREDICTIONS 
 56      menunam='prediobs'
         CALL menu(ipred,menunam,3,'which orbit?=',
     +      'arc 1=','arc 2=',
...
c choice of arc
 58      menunam='multisol'
         CALL menu(iarc,menunam,4,'which orbit?=',
     +      'arc 1=','arc 2=',
     +      'joint computed orbit=',
...
c compute multiple solutions
         IF(iarc.eq.1)THEN
            CALL tee(iun20,'ARC 1=')
            tcmult=t0
            CALL fmulti(obs0,ini0,ok,cov0,tcmult,
     ...
     +           sigma,imult,imim,imip,imi0)
         ELSEIF(iarc.eq.2)THEN
            CALL tee(iun20,'ARC 2=')
            tcmult=tp
            CALL fmulti(obsp,inip,ok,covp,tcmult,
     ...
     +           sigma,imult,imim,imip,imi0)
         ELSEIF(iarc.eq.3)THEN
            CALL tee(iun20,'BOTH ARCS=')
            tcmult=tm
            CALL fmulti(obstwo,initwo,ok,covtwo,tcmult,
...
c ======================================================
 ...
 145     menunam='multiuse'
         CALL menu(ifff,menunam,4,'what to do?=',
...
c a-e plot of multiple solutions
            IF(iarc.eq.1)THEN
               ...
               ELSE
                  tc=tcmult
               ENDIF
               CALL fmuplo(eqm(1,imim),tc,nmult,eq0,titnam,sigma)
            ELSEIF(iarc.eq.2)THEN
               IF(propti)THEN
                  tc=trmult
               ELSE
                  tc=tcmult
               ENDIF
               CALL fmuplo(eqm(1,imim),tc,nmult,eqp,titnam,sigma)
            ELSEIF(iarc.eq.3)THEN
               IF(propti)THEN
                  tc=trmult
               ELSE
                  tc=tcmult
               ENDIF
               CALL fmuplo(eqm(1,imim),tc,nmult,eq,titnam,sigma)
            ENDIF
c ======================================================
...
c only alpha, delta, magnitude
            IF(iarc.eq.1)THEN
               CALL fmuobs(tcmult,gma0,iob1,ids,t1,tut1,sigma,eqm,hmu,
     +         aln(im),den(im),iff,imim,imip,imi0,titnam,filnam,iun20)
            ELSEIF(iarc.eq.2)THEN
               CALL fmuobs(tcmult,gmap,iob1,ids,t1,tut1,sigma,eqm,hmu,
     +         aln(im),den(im),iff,imim,imip,imi0,titnam,filnam,iun20)
            ELSEIF(iarc.eq.3)THEN
               CALL fmuobs(tcmult,gmam,iob1,ids,t1,tut1,sigma,eqm,hmu,
     +         aln(im),den(im),iff,imim,imip,imi0,titnam,filnam,iun20)
            ENDIF
c ======================================================
...
c copy state vector and matrices, norms
     ...
     +                    csinom(imi),delnom(imi))
                     h0=hmu(imi)
                  ELSEIF(iarm.eq.2)THEN
                     tp=tcmult
                     CALL stacop(icop,eqp,gp,cp,csinop,delnop,
     +                    eqm(1,imi),gm(1,1,imi),cm(1,1,imi),
     +                    csinom(imi),delnom(imi))
                     hp=hmu(imi)
                  ELSEIF(iarm.eq.3)THEN
                     tm=tcmult
                     CALL stacop(icop,eq,g,c,csinor,delnor,
     +                    eqm (1,imi),gm(1,1,imi),cm(1,1,imi),
     +                    csinom(imi),delnom(imi))
...
c check availability of JPL ephemerides
            IF(trmult.lt.tejpl1.or.trmult.gt.tejpl2)THEN
               WRITE(*,*)' JPL ephemerides not available for tr=',trmult
...
c =====================================================================
c new feature: close approach analysis
c =====================================================================
         CALL tee(iun20,'CLOSE APPROACH ANALYSIS=')
 558     menunam='closapan'
         CALL menu(iclan,menunam,3,'for which orbit?=',
     +      'arc 1=','arc 2=',
     +      'joint computed orbit=',
     +      s4,s5,s6,s7,s8,s9,s10)
...
c =====================================================================
         ...
         IF(iclan.eq.1)THEN
            ...
            IF(ok)THEN
               CALL tee(iun20,'ARC 1=')
               CALL fclan(batchcl,tlim,eq0,t0,g0,c0,csino0,delno0,iun20,
     +              ok,h0,gma0,obs0,m,objid,iobs,tau,tut,idsta,aln,den,
     +              rmsa,rmsd,sel,astna0)
         ...
         ELSEIF(iclan.eq.2)THEN
            ...
            IF(ok)THEN
               CALL tee(iun20,'ARC 2=')
               CALL fclan(batchcl,tlim,eqp,tp,gp,cp,csinop,delnop,iun20,
     +              ok,hp,gmap,obsp,mp,objid(m+1),iobs(m+1),tau(m+1),
     +              tut(m+1),idsta(m+1),
     ...
     +              ,astnap)
         ...
         ELSEIF(iclan.eq.3)THEN
            ...
            IF(ok)THEN
               CALL tee(iun20,'BOTH ARCS=')
               CALL fclan(batchcl,tlim,eq,tm,g,c,csinor,delnor,iun20,
     +              ok,hm,gmam,obstwo,mall,objid,iobs,tau,tut,idsta,
     +              aln,den,rmsa,rmsd,sel,astna0)
            ENDIF
         ENDIF
         IF(.not.ok)GOTO 558
c =====================================================================
