...
c check requirements
      IF(.not.cov0)THEN
         write(99,*)' covariance matrix for arc 1 not ready'
         ff=.false.
         RETURN
      ENDIF
      IF(.not.covp)THEN
         write(99,*)' covariance matrix for arc 2 not ready'
         ff=.false.
         RETURN
      ENDIF
      IF(t0.ne.tp)THEN
         write(99,*)' to=',t0,' tp=',tp
         write(99,*)' elements and covariances must be available'
         write(99,*)' for the same time, use propagation first'
         RETURN
      ENDIF
...
c identification by 6x6 matrix
     ...
     +    d2,da2,dq,dqalt,dista,detc2,detc6,eqf,fail,eigen6)
      ELSE
         write(99,*)' fident: igue=',igue,' not understood'
         RETURN
      ENDIF
c output
      IF(fail)THEN
      ...
      ENDIF
c penalties
      write(99,*)' d2, d2alt,  dq,   dqalt,    dista'
      write(99,194)d2,da2,dq,dqalt,dista  
