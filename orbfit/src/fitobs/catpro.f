...
c check availability of JPL ephemrides
      IF(tref.lt.tejpl1.or.tref.gt.tejpl2)THEN
         write(99,*)' JPL ephemerides not available for t=',t
...
c read the name of the elements file, inquire
     ...
     +     fail1,fail)
      ...
      write(99,*)' file =',catnam0(1:l0),' exists=',ex,' found=',found,
     +      ' fail=',fail
...
c check availability of covariance and normal matrices
         IF(icov.eq.2)THEN
            IF(.not.defcov.or.(.not.defnor))THEN
               write(99,*)' matrix missing for ',name,defcov,defnor
               GOTO 10
            ENDIF
         ENDIF
c ================================================================
...
c output single line catalog record
         ...
         ndone=ndone+1
 10   ENDDO
      write(99,*) 'CANNOT COMPLETE CATALOG, nx=', nx
c     end of input file
 5    CALL clorbf
      i=i-1
      write(99,*)' total number of orbits ',i,' propagated ',ndone
      stop
      end
