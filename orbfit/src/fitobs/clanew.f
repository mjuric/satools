...
c control on the size of displacement s1 (in AU)
      ...
      IF(abs(s1).gt.s1max)THEN
         write(99,*)' clanew: Newton step too long s1=',s1,s1max
         s1=s1*s1max/abs(s1)
      ENDIF
c control on the size of change in the sigma space
      IF(s1/sig(2).gt.siglim)THEN
         write(99,*)' clanew: too large deltasig=',s1/sig(2),siglim
         error=.true.
         RETURN 
      ENDIF
c new target point on MTP
      ...
      write(99,*)' displ.=',s1,' deltasig=',s1/sig(2),' dmin=',dminew
...
c store new target plane; batch mode
      batchsel=.true.
c      write(99,*)'tcl.tcl0.tcl1.tcl2 ',tcl,tcl0,tcl1,tcl2
      CALL mtpsel(batchsel,tc,t1,xc0,vc0,tcl0,tcl1,tcl2,error)
      IF(error)THEN
          write(99,*)' tc,t1,tcl0,tcl1,tcl2 ',tc,t1,tcl0,tcl1,tcl2
          RETURN
      ENDIF
      dmin0=vsize(xc0)
      vmin0=vsize(vc0)
...
c adopt new minimum as nominal
      ...
      csinor=csinew
      delnor=delnew
      write(99,*)' Newton control=',dminew-tpr
